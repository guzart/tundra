...
RSpec.describe UserTokenController, type: :controller do
  describe 'POST index' do
    ...
    it 'returns a JWT token when submitting valid credentials' do
      post :create, auth: { email: user.email, password: 'helloworld' }
    ...
    it 'fails when submitting invalid credentials' do
      post :create, auth: { email: user.email, password: 'thisisbad' }
